...
contract OwnableERC4626Test is DSTestPlus {
    MockERC20 underlying;
    OwnableERC4626 vault;

    function setUp() public {
        underlying = new MockERC20("Mock Token", "TKN", 18);
        vault = new OwnableERC4626(underlying, "Mock Token Vault", "vwTKN");
    }
    ...
    function testMetadata(string calldata name, string calldata symbol) public {
        OwnableERC4626 vlt = new OwnableERC4626(underlying, name, symbol);
        assertEq(vlt.name(), name);
        assertEq(vlt.symbol(), symbol);
        assertEq(address(vlt.si_asset()), address(underlying));
    }
